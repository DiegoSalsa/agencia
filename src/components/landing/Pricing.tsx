...
export default function Pricing() {
  ...
  return (
    <section id="planes" ref={ref} className="relative py-32 px-6 bg-[var(--bg)]">
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          custom={0}
        >
          <p className="text-xs font-bold tracking-widest uppercase text-[var(--text-tertiary)] mb-4">
            {t('pricing_tag')}
          </p>
        ...
        {promo && (
          ...
          >
            <Link
              ...
              className="group block rounded-none border border-[var(--border)] bg-[var(--bg-secondary)] p-8 md:p-12 hover:border-[rgba(var(--primary-rgb),0.3)] transition-colors duration-300 relative overflow-hidden"
            >
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
                <div>
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold text-[var(--text)] mb-3 tracking-tight">
                    {promo.title}
                  </h3>
                  {promo.description && (
                    <p className="text-[var(--text-secondary)] text-sm mb-4 max-w-lg">{promo.description}</p>
                  )}
                  <div className="flex items-center gap-4 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider">
                ...
                <div className="flex flex-col items-start md:items-end gap-4 shrink-0">
                  <div className="text-left md:text-right">
                    <div className="text-sm text-[var(--text-tertiary)] line-through mb-1">
                      ${promo.originalPrice.toLocaleString('es-CL')}
                    </div>
                    <div className="flex items-baseline gap-2">
                      <span className="text-4xl font-black text-[var(--text)] tracking-tight">
                        ${promo.price.toLocaleString('es-CL')}
                      </span>
                      <span className="text-xs text-[var(--text-tertiary)] font-medium">CLP</span>
                    </div>
                  </div>
                      ...
                      {isEnterprise ? (
                        ...
                        <>
                          ...
                          <div className="text-[10px] text-[#444] uppercase tracking-widest">
                            {t('pricing_payment_note')} <span className="text-[var(--text-tertiary)] font-bold mt-1 block">+ IVA (No incluido)</span>
                          </div>
                        </>
                      )}
                    </div>
